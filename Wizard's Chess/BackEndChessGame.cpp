...
public:
	...
	void GetNextMove(CAPiece* qpaaBoard[8][8]) {
		...
		do {
			...
			cout << mcPlayerTurn << "'s Starting coordinates: ";
			int iStartMove;
			cin >> iStartMove;
			int iStartRow = (iStartMove % 10) - 1;
			int iStartCol = (iStartMove / 10) - 1;

			cout << mcPlayerTurn << "'s Destination coordinates: ";
			int iEndMove;
			cin >> iEndMove;
			...
			std::cout << "CHECK 1 " << iStartRow << "," << iStartCol << "\n";
				...
				if ((qpCurrPiece != 0) && (qpCurrPiece->GetColor() == mcPlayerTurn)) {
					...
					if (qpCurrPiece->IsLegalMove(iStartRow, iStartCol, iEndRow, iEndCol, qpaaBoard)) {
						...
						if (!mqGameBoard.IsInCheck(mcPlayerTurn)) {
							delete qpTemp;
							bValidMove = true;
							std::cout << "CHECK 2 " << iStartRow << "," << iStartCol << "\n";
							b.moveDirector(iStartCol+1, iStartRow+1, iEndCol+1, iEndRow+1);
						}
